...
sanger_rep_seqs=$data_dir/Soil_isolates_Sanger.fasta
WGS_rep_seqs=$data_dir/Soil_isolates_WGS.fasta

# prepare results directory

results_dir=$working_dir/recovery_rates_soil
mkdir -p $results_dir
rm -f -r $results_dir/*
...
formatdb -n $results_dir/blastDB_soil \
         -i $target \
         -p false \
         &>> $output
...
blastall -p blastn \
         ...
         -b 100000 \
         -i $query \
         -d $results_dir/blastDB_soil \
         -o $results_dir/blast_results_soil.txt \
         &>> $output

# cleanup
rm -f blastDB_soil* formatdb.log error.log
...
$scripts_dir/recovery_rates_soil.R $working_dir \
                                   $results_dir \
                                   $min_length \
                                   $min_id \
                                   &>> $output
