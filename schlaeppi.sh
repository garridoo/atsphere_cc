...
working_dir=$working_dir/schlaeppi
mkdir -p $working_dir
rm -f -r $working_dir/*
...
# were retrived from the European Nucleotide Archive database
# (accession no. PRJEB5058). Subsequently, only A. thaliana 
...
truncate_fasta_qual_files.py -f $data_dir/schlaeppi/all.fasta \
                             -q $data_dir/schlaeppi/all.qual \
                             -b $t_length \
                             -o $working_dir/trunc \
                             &>> $output
...
sampleSizes $working_dir/seqs.fasta \
            $data_dir/schlaeppi/"$l"_mapping.txt \
