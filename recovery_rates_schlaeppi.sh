...
schlaeppi_rep_seqs=$working_dir/schlaeppi/rep_seqs.fasta
...
results_dir=$working_dir/recovery_rates_schlaeppi
mkdir -p $results_dir
rm -f -r $results_dir/*
...
cat $schlaeppi_rep_seqs | sed 's/>/>schlaeppi_/g' >> $results_dir/schlaeppi_rep_seqs.fasta
...
query=$results_dir/schlaeppi_rep_seqs.fasta
...
blastall -p blastn \
         ...
         -o $results_dir/blast_results_schlaeppi.txt \
         &>> $output
...
$scripts_dir/recovery_rates_schlaeppi.R $working_dir \
                                        $results_dir \
                                        $min_length \
                                        $min_id \
                                        &>> $output
